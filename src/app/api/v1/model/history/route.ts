...
import { ModelHistoryQuerySchema } from "@/modules/model-episodes/model-episode.schema";
import { getModelHistory } from "@/modules/model-episodes/model-episode.service";
...
export async function GET(request: Request): Promise<Response> {
  ...
  const parsed = ModelHistoryQuerySchema.safeParse(
    Object.fromEntries(new URL(request.url).searchParams),
  );
  if (!parsed.success) return validationResponse(parsed.error);
  try {
    return Response.json(await getModelHistory(parsed.data));
  } catch (error) {
