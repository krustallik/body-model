...
const EMPTY_DIAGNOSTICS: DiagnosticsDto = {
  episode: {
    id: 0,
    modelVersion: "not-created",
  ...
  currentState: {
    ...
    fatMassKg: null,
    leanTissueKg: null,
    glycogenAndExtracellularFluidMassKg: null,
    dynamicRmrKcalPerDay: null,
    modeledTdeeKcalPerDay: null,
  },
  experimentalEnergySummary: null,
  dataContinuity: {
    level: "blocked",
    recentWindowDays: 28,
    ...
    nutrition: { observedDayCount: 0, imputedDayCount: 0, unresolvedDayCount: 0 },
  ...
  personalization: {
    ...
    initialization: {
      ...
      confidence: "not-initialized",
      appliedCorrectionKcalPerDay: 0,
      applied: false,
      explanation: "The production model has not been initialized yet.",
    },
    evidence: {
      completeDayCount: null,
      observationCount: null,
      observationSpanDays: null,
      activityStandardDeviationKcalPerDay: null,
      activityCoefficientOfVariation: null,
    },
    gates: [
      { id: "offset-observations", current: null, required: 20, unit: "observations", met: false },
      { id: "offset-span", current: null, required: 28, unit: "days", met: false },
      { id: "full-observations", current: null, required: 35, unit: "observations", met: false },
      { id: "full-span", current: null, required: 56, unit: "days", met: false },
      { id: "activity-standard-deviation", current: null, required: 75, unit: "kcal/day-sd", met: false },
      { id: "activity-coefficient-of-variation", current: null, required: 0.2, unit: "coefficient-of-variation", met: false },
    ],
    nextGate: null,
    warnings: [],
  },
  ...
  forecastReadiness: {
    ...
    reasons: ["current-state-unavailable"],
  },
  limitations: [
    ...
    { id: "parameter-uncertainty-not-modeled", scope: "model" },
    { id: "hold-ecf", scope: "current-state" },
    { id: "long-horizon-numerical-quality", scope: "forecast" },
  ],
};

type DiagnosticsRequestError = Error & { code?: string };
...
export function DiagnosticsClient() {
  ...
  const noActiveCopy = noActiveModelPresentation(locale);
  const [data, setData] = useState<DiagnosticsDto | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [noActiveModel, setNoActiveModel] = useState(false);
  ...
  const loadDiagnostics = useCallback(async (signal?: AbortSignal) => {
    ...
    if (!response.ok) {
      const failure = new Error(response.status === 404
        ? (uk ? "Немає активної моделі." : "No active model.")
        : (uk ? "Не вдалося завантажити діагностику." : "Could not load diagnostics."));
      if (response.status === 404) failure.name = "NoActiveModelDiagnosticsError";
      throw failure;
    }
  ...
  useEffect(() => {
    ...
    loadDiagnostics(controller.signal)
      ...
      .catch((cause: unknown) => {
        ...
        const failure = cause as DiagnosticsRequestError;
        if (failure.name === "NoActiveModelDiagnosticsError") {
          setData(EMPTY_DIAGNOSTICS);
          setNoActiveModel(true);
        }
  ...
  async function runModelAction(action: "initialize" | "recalculate") {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(action === "initialize"
          ? initializationFailureMessage(body.reason ?? body.message, locale)
          : (uk ? "Не вдалося перерахувати модель." : "Could not recalculate the model."));
      }
      setData(await loadDiagnostics());
      setNoActiveModel(false);
    } catch (cause: unknown) {
      const failure = cause as DiagnosticsRequestError;
      if (failure.name === "NoActiveModelDiagnosticsError") {
        setData(EMPTY_DIAGNOSTICS);
  ...
  function runRecalculate() {
    return runModelAction(noActiveModel ? "initialize" : "recalculate");
  }
  ...
  const needsRecalculation = data
    ...
    : false;
  const actionLabel = noActiveModel ? noActiveCopy.primaryAction : recalculateCopy.action;
  const actionLoadingLabel = noActiveModel ? noActiveCopy.loadingAction : recalculateCopy.loadingAction;
  ...
  return <main className={styles.page}>
    ...
    {error && <section className={styles.error} role="status"><strong>{noActiveModel ? (uk ? "Модель ще не створена" : "Model not created yet") : (uk ? "Діагностика недоступна" : "Diagnostics unavailable")}</strong><span>{error}</span><div className={styles.actions}><button type="button" disabled={recalculating} aria-busy={recalculating} onClick={() => void runRecalculate()}>{recalculating ? actionLoadingLabel : actionLabel}</button><Link href="/dashboard">{uk ? "Перейти до огляду" : "Go to dashboard"}</Link><Link href="/history">{uk ? "Додати дані" : "Add data"}</Link></div></section>}
    {data && <>
      <section className={styles.recalculateBanner} aria-label={actionLabel}>
        <div><strong>{noActiveModel ? (uk ? "Активної моделі ще немає" : "There is no active model yet") : needsRecalculation ? (uk ? "Порахованих днів ще немає" : "No calculated days yet") : (uk ? "Оновити пораховану історію" : "Refresh calculated history")}<HelpTip>{uk ? "Кнопка бере записи з таблиці «Історія», заново рахує фізіологічну модель, зберігає результат і за потреби сама оцінює стан після пропуску для Діагностики, Прогнозу та Цілі. Самі записи здоров’я вона не змінює." : "This reads Health History, recalculates the physiology model, saves results, and when needed estimates state after a data gap for Diagnostics, Forecast, and Goal. It does not edit health records."}</HelpTip></strong><span>{noActiveModel ? noActiveCopy.detail : recalculateCopy.hint}</span>{error && noActiveModel && <span>{error}</span>}</div>
        <button type="button" className={needsRecalculation || noActiveModel ? styles.recalculatePrimary : styles.recalculateSecondary} disabled={recalculating} aria-busy={recalculating} onClick={() => void runRecalculate()}>{recalculating ? actionLoadingLabel : actionLabel}</button>
      </section>
      ...
      <section className={styles.overview} aria-label={uk ? "Огляд стану моделі" : "Model status overview"}>
        ...
        <article data-level={data.dataContinuity.level}><div className={styles.cardTop}><span>{uk ? "Дані · до 28 днів" : "Data · up to 28 days"}<HelpTip>{uk ? "Перше число — дні, які модель змогла повністю порахувати; друге — усі збережені модельні дні у вікні. Це не кількість рядків у таблиці здоров’я." : "The first number is fully calculated days; the second is all saved model days in the window. This is not the row count in Health History."}</HelpTip></span><b>{levelLabel(data.dataContinuity.level, uk)}</b></div><strong>{data.episode.id === 0 ? (uk ? "Історію ще не пораховано" : "History not calculated yet") : `${data.dataContinuity.completeDayCount}/${data.dataContinuity.modeledDayCount} ${uk ? "повних порахованих днів" : "complete calculated days"}`}</strong><p>{data.episode.id === 0 ? (uk ? "Сирі записи ще не стали модельними днями." : "Raw records have not been turned into model days yet.") : `${uk ? "Зважувань" : "Weigh-ins"}: ${data.dataContinuity.weightObservationCount} · ${uk ? "днів без калорій" : "days without calories"}: ${data.dataContinuity.nutrition.unresolvedDayCount}`}</p></article>
      ...
      <section className={styles.grid}>
        ...
        <article className={styles.panel}><p className={styles.eyebrow}>{uk ? "Звідки взялись дані" : "Where the data came from"}<HelpTip>{uk ? "Показує, скільки днів у вибраному періоді мають ваш запис калорій, скільки модель обережно заповнила за сусідніми днями та скільки залишилися без даних про харчування." : "Shows how many days in the selected period use your calorie log, were cautiously filled from nearby days, or still have no nutrition data."}</HelpTip></p><h2>{uk ? "Дані про харчування за період" : "Nutrition data for the period"}</h2><div className={styles.provenance}><span><i data-kind="observed" />{uk ? "Записано вами" : "Logged by you"}<b>{data.episode.id === 0 ? "—" : data.dataContinuity.nutrition.observedDayCount}</b></span><span><i data-kind="imputed" />{uk ? "Підставлено з сусідніх днів" : "Filled from nearby days"}<b>{data.episode.id === 0 ? "—" : data.dataContinuity.nutrition.imputedDayCount}</b></span><span><i data-kind="missing" />{uk ? "Без калорій" : "Missing calories"}<b>{data.episode.id === 0 ? "—" : data.dataContinuity.nutrition.unresolvedDayCount}</b></span></div><p className={styles.note}>{data.episode.id === 0 ? (uk ? "Це діагностика production-моделі. Сирі записи можна переглянути в Історії; після запуску моделі тут з’явиться їхній розподіл." : "This is the production-model diagnostic. Raw records are available in History; their breakdown will appear here after the model starts.") : (uk ? "Якщо за день не записано окремий робочий інтервал, модель вважає, що того дня не було додаткової активності на роботі. Інші записи цього дня — харчування, кроки, тренування та вага — не вважаються втраченими." : "If no separate work interval is logged for a day, the model assumes there was no additional occupational activity. Other records for that day—nutrition, steps, workouts, and weight—are not treated as missing.")}</p></article>
      </section>
