  ...
  return <main className={styles.page}>
    ...
    {!loadingContext && initialized && canPlan && latestModeledDate && <form className={styles.planner} onSubmit={(event) => void submit(event)} noValidate>
      ...
      <section className={styles.formSection}><div className={styles.sectionHeading}><div><span>04</span><h2>{uk ? "Майбутня активність" : "Future activity"}</h2></div><p>{uk ? "Активність фіксується вашим сценарієм і не оптимізується разом із калоріями." : "Activity is fixed by your scenario and is not optimized together with calories."}</p></div><div className={styles.modeGrid}><button type="button" aria-pressed={form.mode === "target-centered"} onClick={() => updateForm("mode", "target-centered")}><strong>{uk ? "Гнучкий сценарій" : "Flexible scenario"}</strong><span>{uk ? "Майбутня поведінка варіюється навколо плану." : "Future behavior varies around the plan."}</span></button><button type="button" aria-pressed={form.mode === "fixed"} onClick={() => updateForm("mode", "fixed")}><strong>{uk ? "Точний сценарій" : "Exact scenario"}</strong><span>{uk ? "План повторюється без варіації дотримання." : "The plan repeats without adherence variation."}</span></button></div><div className={styles.formGrid}>
        <PlanNumberField id="averageSteps" label={uk ? "Середня кількість кроків" : "Average steps"} unit={uk ? "на день" : "per day"} help={uk ? "Одна середня кількість кроків для всіх днів — без поділу на робочі та неробочі. Для розрахунку модель переводить кроки у приблизну дистанцію." : "One daily step average for all days, without splitting work and non-work days. The model converts steps to an approximate distance."} value={form.plan.averageStepsPerDay} max={100000} step={100} onChange={(value) => updatePlan("averageStepsPerDay", value)} />
        <PlanNumberField id="strengthDays" label={uk ? "Силових тренувань" : "Strength sessions"} unit={uk ? "на тиждень" : "per week"} value={form.plan.strengthDaysPerWeek} max={7} onChange={(value) => updatePlan("strengthDaysPerWeek", value)} />
        ...
        <PlanNumberField id="otherTrainingDays" label={uk ? "Інших тренувань" : "Other sessions"} unit={uk ? "на тиждень" : "per week"} value={form.plan.otherTrainingDaysPerWeek} max={7} onChange={(value) => updatePlan("otherTrainingDaysPerWeek", value)} />
        <PlanNumberField id="otherTrainingMinutes" label={uk ? "Тривалість іншого тренування" : "Other training duration"} unit={uk ? "хв" : "min"} help={uk ? "Оскільки тип та інтенсивність не вказуються, модель використовує тривалість як приблизну загальну тренувальну активність." : "Because type and intensity are not specified, the model uses duration as an approximate general training activity."} value={form.plan.otherTrainingMinutes} max={600} onChange={(value) => updatePlan("otherTrainingMinutes", value)} />
      </div><label className={styles.toggle}><input type="checkbox" checked={form.plan.plannedWork} onChange={(event) => updatePlan("plannedWork", event.currentTarget.checked)} /><span>{uk ? "Додати робочі дні" : "Include work days"}</span></label>{form.plan.plannedWork && <div className={styles.formGrid}><PlanNumberField id="workDaysPerWeek" label={uk ? "Робочих днів" : "Work days"} unit={uk ? "на тиждень" : "per week"} value={form.plan.workDaysPerWeek} min={1} max={7} onChange={(value) => updatePlan("workDaysPerWeek", value)} /><label className={styles.field} htmlFor="workCategory"><span>{uk ? "Інтенсивність роботи" : "Work intensity"}</span><select id="workCategory" value={form.plan.workCategory} onChange={(event) => updatePlan("workCategory", event.currentTarget.value as PlanValues["workCategory"])}><option value="standingLight">{uk ? "Дуже легка" : "Very light"}</option><option value="manualLight">{uk ? "Легка ручна" : "Light manual"}</option><option value="standingLightModerate">{uk ? "Активна легка" : "Active light"}</option><option value="manualModerate">{uk ? "Помірна ручна" : "Moderate manual"}</option></select></label><PlanNumberField id="shiftHours" label={uk ? "Зміна" : "Shift"} unit={uk ? "год" : "hours"} value={form.plan.shiftHours} min={0.1} max={24} step={0.25} onChange={(value) => updatePlan("shiftHours", value)} /><PlanNumberField id="breakHours" label={uk ? "Перерви" : "Breaks"} unit={uk ? "год" : "hours"} value={form.plan.breakHours} max={form.plan.shiftHours} step={0.25} onChange={(value) => updatePlan("breakHours", value)} /></div>}<FieldError id="plan-error" message={formErrors.plan} /></section>
    ...
    {result && <>
      ...
      <section className={styles.detailGrid}><article><h2>{uk ? "Введені припущення" : "Submitted assumptions"}</h2><dl><div><dt>{uk ? "Ціль" : "Target"}</dt><dd>{result.goal.targetValueKg} kg · {result.goal.goalDate}</dd></div><div><dt>{uk ? "Межі калорій" : "Calorie bounds"}</dt><dd>{result.assumptions.constraints.minCaloriesKcal}–{result.assumptions.constraints.maxCaloriesKcal} kcal</dd></div><div><dt>{uk ? "Сценарій" : "Scenario"}</dt><dd>{result.assumptions.scenarioMode}</dd></div><div><dt>{uk ? "Шаблон" : "Template"}</dt><dd>{result.assumptions.referenceNutrition.caloriesKcal} kcal · P {result.assumptions.referenceNutrition.proteinG} · F {result.assumptions.referenceNutrition.fatG} · C {result.assumptions.referenceNutrition.carbsG}</dd></div><div><dt>{uk ? "Середні кроки" : "Average steps"}</dt><dd>{new Intl.NumberFormat(uk ? "uk-UA" : "en-US").format(form.plan.averageStepsPerDay)} / {uk ? "день" : "day"}</dd></div><div><dt>{uk ? "Тренування" : "Training"}</dt><dd>{form.plan.strengthDaysPerWeek} × {form.plan.strengthTrainingMinutes} {uk ? "хв силових" : "min strength"} · {form.plan.otherTrainingDaysPerWeek} × {form.plan.otherTrainingMinutes} {uk ? "хв інших" : "min other"}</dd></div><div><dt>{uk ? "Робочі дні" : "Work days"}</dt><dd>{form.plan.plannedWork ? `${form.plan.workDaysPerWeek} / ${uk ? "тиждень" : "week"}` : "—"}</dd></div></dl></article><article><h2>{uk ? "Якість і обмеження" : "Quality and limitations"}</h2><ul><li>{uk ? "Тип початкової оцінки" : "Initial estimate type"}: {result.provenance.initialStateQuality ? <ModelStateSource value={result.provenance.initialStateQuality} uk={uk} /> : "—"}</li><li>{uk ? "Якість Forecast" : "Forecast quality"}: {result.numerical.forecastQuality ?? "—"}</li><li>{uk ? "Числова похибка солвера" : "Solver tolerance"}: {result.numerical.solverToleranceKg} kg</li><li>{uk ? "Смуга near-target probability" : "Near-target probability band"}: ±{result.numerical.goalToleranceKg} kg ({uk ? "інженерне налаштування" : "engineering setting"})</li><li>{uk ? "Локальна чутливість" : "Local sensitivity"}: {result.numerical.localSensitivityKgPer100Kcal === null ? "—" : `${result.numerical.localSensitivityKgPer100Kcal.toFixed(2)} kg / 100 kcal`}</li>{result.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul></article></section>
    </>}
  </main>;
}
