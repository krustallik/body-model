...
export function AppNav({ active }: { active: "dashboard" | "history" | "training" | "forecast" | "goal" | "diagnostics" | "profile" }) {
  ...
  return (
    <nav className={styles.nav} aria-label={labels.aria}>
      ...
      <Link className={active === "training" ? styles.active : undefined} href="/training">{labels.training}</Link>
