...
export function AppNav({ active }: { active: "dashboard" | "history" | "forecast" | "goal" | "profile" }) {
  const { locale } = useI18n();
  const labels = locale === "uk"
    ? { dashboard: "Огляд", history: "Історія", forecast: "Прогноз", goal: "Ціль", profile: "Профіль", aria: "Основна навігація" }
    : { dashboard: "Dashboard", history: "History", forecast: "Forecast", goal: "Goal", profile: "Profile", aria: "Primary navigation" };
  return (
