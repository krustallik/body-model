...
export const PHYSIOLOGY_V7_CONTRACT_VERSION = "bodycast-physiology-v7-state-v3" as const;
...
export type PhysiologyV7State = {
  ...
  adaptiveThermogenesisKcalPerDay: AdaptiveThermogenesisState["adaptiveThermogenesisKcalPerDay"] | null;
  /** Retained only when a prior v7 state actually provides it. */
  weightFilterState: WeightFilterState | null;
};
...
export function validatePhysiologyV7State(state: PhysiologyV7State): PhysiologyV7State {
  for (const field of NONNEGATIVE_COMPARTMENTS) {
    ...
    if (!Number.isFinite(value)) throw new TypeError(`${field} must be finite when available`);
  ...
  if (state.adaptiveThermogenesisKcalPerDay !== null
      && !Number.isFinite(state.adaptiveThermogenesisKcalPerDay)) {
    throw new TypeError("adaptiveThermogenesisKcalPerDay must be finite when available");
  }
  if (state.weightFilterState !== null) {
...
export function reconstructPhysiologyV7MassKg(state: PhysiologyV7State): number | null {
  validatePhysiologyV7State(state);
  if (state.fatMassKg === null || state.skeletalMuscleKg === null
      || state.otherLeanTissueKg === null || state.glycogenKg === null
      || state.glycogenWaterKg === null || state.ecfDeviationKg === null
      || state.transientExerciseWaterKg === null) return null;
