...
describe("scientific v7 traceability manifest", () => {
  it("accounts for exactly 80 eligible audited claims with stable unique IDs", () => {
    expect(SCIENTIFIC_V7_CLAIMS).toHaveLength(80);
    expect(new Set(SCIENTIFIC_V7_CLAIMS.map(({ claimId }) => claimId)).size).toBe(80);
  });
  ...
  it("reports separate GREEN / EXPERIMENTAL / BLOCKED / REJECTED counts", () => {
    ...
    expect(summary.GREEN + summary.EXPERIMENTAL + summary.BLOCKED).toBe(80);
    expect(summary.GREEN + summary.EXPERIMENTAL + summary.BLOCKED + summary.REJECTED).toBe(84);
  });
  ...
  it("keeps REJECTED claim IDs out of eligible GREEN/BLOCKED/EXPERIMENTAL sets", () => {
    ...
    expect(ALL_SCIENTIFIC_V7_CLAIM_RECORDS).toHaveLength(84);
  });
  ...
  it("does not blanket-convert BLOCKED claims into EXPERIMENTAL without implementations", () => {
    ...
    expect(SCIENTIFIC_V7_CLAIMS.filter((claim) => claim.status === "BLOCKED")).toHaveLength(0);
  });
});
