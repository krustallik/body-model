...
const v7HypertrophyResponseBlocker = "Mapped hard-set dose exists, but no skeletal-muscle / hypertrophy response output exists against which a universal set cutoff could force zero or negative gain.";
...
export const SCIENTIFIC_V7_CLAIMS: readonly ScientificClaimManifestRecord[] = [
  ...
  record({ claimId: "C-C05", title: "strength loss is not muscle loss", parameterIds: ["P-C03"], evidenceIds: ["E-C03", "E-B13"], auditEligibility: "SAFE", testType: "unit", assertionTypes: ["INVARIANT"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "Strength decline cannot be converted directly into skeletal-muscle loss.", infrastructureBlocker: v7MeasurementBlocker }),
  ...
  record({ claimId: "C-F01", title: "strength workouts consume rather than create glycogen", parameterIds: ["P-F01"], evidenceIds: ["E-F01", "E-F02", "E-F03", "E-F04"], auditEligibility: "SAFE", testType: "unit", assertionTypes: ["BOUND"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "The exercise-only strength-workout glycogen transition is nonpositive.", infrastructureBlocker: v7WorkoutGlycogenBlocker }),
  record({ claimId: "C-F02", title: "strength-workout depletion is store bounded", parameterIds: ["P-F01"], evidenceIds: ["E-F01", "E-F02", "E-F03"], auditEligibility: "SAFE", testType: "property", assertionTypes: ["BOUND", "CONSERVATION"], provenance: ["SCIENTIFIC_EVIDENCE", "MODEL_CONSERVATION_RULE"], scientificAssertion: "Workout depletion cannot exceed available glycogen and cannot make the store negative.", infrastructureBlocker: v7WorkoutGlycogenBlocker }),
  record({ claimId: "C-F03", title: "additional contained comparable work does not lower cumulative demand", parameterIds: ["P-F02"], evidenceIds: ["E-F01"], auditEligibility: "SAFE_AFTER_AUDIT_REVISION", testType: "property", assertionTypes: ["MONOTONICITY"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "In an isolated no-refeeding session, a higher-set condition containing all lower-set work plus comparable added work does not have lower cumulative demand.", infrastructureBlocker: v7WorkoutGlycogenBlocker }),
  record({ claimId: "C-F04", title: "muscle recruitment matters for glycogen demand", parameterIds: ["P-F01"], evidenceIds: ["E-F01", "E-F02"], auditEligibility: "SAFE", testType: "unit", assertionTypes: ["INVARIANT"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "Local glycogen demand cannot be applied universally without muscle mapping.", infrastructureBlocker: v7WorkoutGlycogenBlocker }),
  ...
  record({ claimId: "C-H03", title: "daily v7 need not apply meal-frequency effect", parameterIds: ["P-H02"], evidenceIds: ["E-H02", "E-H04"], auditEligibility: "SAFE_AFTER_AUDIT_REVISION", testType: "unit", assertionTypes: ["INVARIANT"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "At daily resolution with no second workout, v7 need not apply a meal-frequency effect; physiological equality is not asserted.", infrastructureBlocker: "Canonical nutrition input has daily totals only and exposes no explicit v7 timing-policy result to test." }),
  record({ claimId: "C-H05", title: "protein is not double-counted as a glycogen bonus", parameterIds: ["P-H03"], evidenceIds: ["E-H01", "E-H06"], auditEligibility: "SAFE", testType: "integration", assertionTypes: ["NO_DOUBLE_COUNTING", "INVARIANT"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "With carbohydrate and energy matched, v7 adds no independent protein glycogen bonus.", infrastructureBlocker: "The v7 nutrition-to-glycogen transition does not expose energy-matched protein substitution scenarios." }),
  ...
  record({ claimId: "C-I03", title: "additional hydration is not automatically glycogen water", parameterIds: ["P-I01"], evidenceIds: ["E-I04", "E-I05"], auditEligibility: "SAFE_AFTER_AUDIT_REVISION", testType: "unit", assertionTypes: ["INVARIANT"], provenance: ["SCIENTIFIC_EVIDENCE"], scientificAssertion: "Water beyond the selected uncertain associated component is not automatically classified as glycogen water; no hard 4 kg/kg boundary is used.", infrastructureBlocker: "No v7 water-observation classifier exposes selected associated water versus other transient compartments." }),
