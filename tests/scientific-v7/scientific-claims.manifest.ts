...
const experimentalChronicSleepAnabolicContextImpl =
  "src/model/physiology-v7/experimental-chronic-sleep-anabolic-context-v1.ts";
const experimentalChronicSleepAnabolicContextTest =
  "tests/experimental-chronic-sleep-anabolic-context-v1.test.ts";
const experimentalChronicSleepAnabolicContextUncertainty =
  "Experimental repeated observed low-sleep context heuristic: three low-duration wearable/manual observations widen only an existing relative skeletal-muscle interval, while one poor night and missing sleep create no point shift; thresholds are not clinical cutoffs, PSG truth, or sleep-to-muscle coefficients.";
...
export const SCIENTIFIC_V7_CLAIMS: readonly ScientificClaimManifestRecord[] = [
  ...
  record({ claimId: "C-M01", title: "severe multi-night restriction creates no positive anabolic bonus", parameterIds: ["P-M01"], evidenceIds: ["E-M01", "E-M02"], auditEligibility: "SAFE_AFTER_AUDIT_REVISION", testType: "longitudinal", assertionTypes: ["BOUND", "MISSINGNESS"], provenance: ["SCIENTIFIC_EVIDENCE", "INPUT_CONTRACT", "ENGINEERING_ASSUMPTION"], scientificAssertion: "Repeated observed low-sleep context may widen experimental uncertainty around an existing relative skeletal-muscle interval but creates no positive anabolic bonus or exact chronic penalty; one poor night and missing sleep do not shift the point estimate.", experimental: { implementation: experimentalChronicSleepAnabolicContextImpl, testFile: experimentalChronicSleepAnabolicContextTest, testName: "does not let one poor night alter the muscle point estimate (C-M01)", uncertainty: experimentalChronicSleepAnabolicContextUncertainty } }),
